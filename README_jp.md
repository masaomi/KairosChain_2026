...
## 利用可能なツール（コア19個 + スキルツール）

基本インストールでは19個のツールが提供されます。`skill_tools_enabled: true`の場合、`kairos.rb`の`tool`ブロックで追加のツールを定義できます。
...
**A:** これは認識されている制限事項です。現状、KairosChainは**自動的なスキルライフサイクル管理**（非推奨化、アーカイブ、クリーンアップ）を持っていません。

**懸念される問題：**

| 問題 | 影響 |
|------|------|
| 検索効率の低下 | スキルが増えるほど関連スキル検索に時間がかかる |
| コンテキストウィンドウの圧迫 | 参照スキルが増えるとLLMの有効トークン数が減る |
| 古いスキルとの矛盾 | 更新されていないスキルが誤った情報を提供 |
| ノイズ比の増加 | 有用なスキルが埋もれる |

**現在の機能：**

- `knowledge_update command="delete"` でL1スキルを削除可能（削除もブロックチェーンに記録）
- L2コンテキストは設計上一時的（セッションスコープ）
- 自動クリーンアップや非推奨化の仕組みは存在しない

**将来の検討事項：**

1. **メタデータによるライフサイクル管理**: `last_accessed`、`access_count`、`status`フィールドの追加
2. **アーカイブレイヤー**: アクティブ検索から除外されるが参照可能なスキル
3. **定期的な棚卸しプロンプト**: エージェント側でクリーンアップを提案するルール

**推奨される回避策（エージェント側の棚卸しプロンプト）：**

AIエージェントに定期的なスキルレビューを設定してください：

```markdown
# スキル棚卸しルール（Cursor Rules / system_prompt用）

## 月次レビューのトリガー
- 毎月初め、またはユーザーが「スキルを見直して」と言った時
- スキル数が閾値を超えた時（例：L1スキルが50個以上）

## 棚卸しプロセス
1. 全L1スキルを最終更新日とともにリスト
2. 30日以上アクセスのないスキルを特定
3. ユーザーに提示：「これらのスキルは最近使われていません：[リスト]」
4. 確認：「アーカイブまたは削除しますか？」

## クリーンアップアクション
- アーカイブ: 別の「archived」ナレッジディレクトリに移動
- 削除: `knowledge_update command="delete" reason="クリーンアップ: 30日以上未使用"`
- 維持: レビュー済みとしてマーク（更新日を更新またはレビュータグを追加）

## レビューフォーマット
「月次スキル棚卸し：L1スキルが[N]個あります。[M]個が30日以上アクセスされていません。
クリーンアップのためにレビューしますか？」
```

このアプローチにより、KairosChainは中立的なインフラストラクチャのまま、エージェントレベルで積極的なメンテナンスが可能になります。

---

### Q: スキルが間違っていたり古くなっていたりした場合、どうやって修正しますか？

**A:** 既存のツールでスキルを修正できますが、ワークフローは明確に整備されていません。
...
**推奨されるエージェント側のフィードバック処理ルール：**

```markdown
# スキル品質フィードバックルール（Cursor Rules / system_prompt用）

## ユーザーがスキルの問題を報告した場合
ユーザーが「これ間違ってる」「古い」「わかりにくい」などと言った場合：

1. 問題のある回答で参照されたスキルを特定
2. `knowledge_get` でスキル内容を表示
3. 何が問題か具体的に確認
4. 修正前後の比較とともに修正案を提案
5. ユーザー承認後、`knowledge_update` を呼び出し：
   - command: "update"
   - reason: "ユーザーフィードバック: [具体的な問題]"

## 積極的な品質チェック
スキルに基づいて回答した後：
「この回答は[skill_name]に基づいています。正しそうですか？
それともスキルを更新した方がいいですか？」
```

**将来の検討事項：**

1. **インラインフィードバック機構**: 「参照されたスキルに問題あり」をワンアクションで報告
2. **自動修正提案**: LLMが文脈に基づいて改善案を提案
3. **変更差分プレビュー**: 更新適用前に並べて比較
4. **フィードバック履歴**: スキルごとのフィードバックを追跡し、問題の多いスキルを特定

この制限は認識されています。現在の設計では明示的なユーザー主導の修正が必要で、人間の監視を維持しますがフィードバックループに摩擦が生じます。
