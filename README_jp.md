...
<!-- 更新方法: KairosChain_mcp_server/knowledge/ 内のL1 knowledgeファイルを編集 -->
<!-- 再生成: ruby scripts/build_readme.rb (または: rake build_readme) -->
...
- [哲学](#哲学)
  - [問題](#問題)
  - [解決策：KairosChain](#解決策：kairoschain)
  - [Minimum-Nomicの原則](#minimum-nomicの原則)
- [アーキテクチャ](#アーキテクチャ)
  - [システム概要](#システム概要)
  - [P2P知識交換](#p2p知識交換)
- [レイヤー化されたスキルアーキテクチャ](#レイヤー化されたスキルアーキテクチャ)
  - [L0：Kairosコア（`skills/`）](#l0：kairosコア（skills）)
  - [L1：知識レイヤー（`knowledge/`）](#l1：知識レイヤー（knowledge）)
  - [L2：コンテキストレイヤー（`context/`）](#l2：コンテキストレイヤー（context）)
  - [なぜレイヤーアーキテクチャか？](#なぜレイヤーアーキテクチャか？)
- [データモデル：SkillStateTransition](#データモデル：skillstatetransition)
- [セットアップ](#セットアップ)
  - [前提条件](#前提条件)
  - [インストール](#インストール)
  - [オプション：RAG（セマンティック検索）サポート](#オプション：rag（セマンティック検索）サポート)
  - [オプション：SQLiteストレージバックエンド（チーム利用向け）](#オプション：sqliteストレージバックエンド（チーム利用向け）)
  - [オプション：Streamable HTTPトランスポート（リモート/チームアクセス）](#オプション：streamable-httpトランスポート（リモートチームアクセス）)
  - [管理者UI（ブラウザベースの管理画面）](#管理者ui（ブラウザベースの管理画面）)
- [クライアント設定](#クライアント設定)
  - [Claude Code設定（詳細）](#claude-code設定（詳細）)
  - [Cursor IDE設定（詳細）](#cursor-ide設定（詳細）)
- [Gemのアップグレード](#gemのアップグレード)
  - [仕組み](#仕組み)
  - [L1知識のアップデート（v1.0.0以降）](#l1知識のアップデート（v100以降）)
  - [アップグレードコマンド](#アップグレードコマンド)
  - [バージョン不一致の警告](#バージョン不一致の警告)
  - [アップグレードワークフロー](#アップグレードワークフロー)
- [MMP SkillSetセットアップ（P2P通信）](#mmp-skillsetセットアップ（p2p通信）)
  - [MMP SkillSetのインストール](#mmp-skillsetのインストール)
  - [meeting.ymlの設定](#meetingymlの設定)
  - [P2P HTTPサーバーの起動](#p2p-httpサーバーの起動)
  - [P2P接続テスト](#p2p接続テスト)
- [セットアップのテスト](#セットアップのテスト)
  - [1. 基本的なコマンドラインテスト](#1-基本的なコマンドラインテスト)
  - [2. スキルツールテスト](#2-スキルツールテスト)
  - [3. ブロックチェーンツールテスト](#3-ブロックチェーンツールテスト)
  - [4. SQLiteバックエンドのテスト（オプション）](#4-sqliteバックエンドのテスト（オプション）)
  - [5. RAG / セマンティック検索のテスト（オプション）](#5-rag-セマンティック検索のテスト（オプション）)
  - [6. HTTPモードのテスト（オプション）](#6-httpモードのテスト（オプション）)
  - [7. Claude Codeでのテスト](#7-claude-codeでのテスト)
  - [8. Cursorでのテスト](#8-cursorでのテスト)
...
- [利用可能なツール（コア26個 + スキルツール）](#利用可能なツール（コア26個-スキルツール）)
  ...
  - [L1：知識ツール - ハッシュ参照記録](#l1：知識ツール-ハッシュ参照記録)
  - [L2：コンテキストツール - ブロックチェーン記録なし](#l2：コンテキストツール-ブロックチェーン記録なし)
  - [ブロックチェーンツール](#ブロックチェーンツール)
  - [StateCommitツール（監査可能性向上）](#statecommitツール（監査可能性向上）)
  - [認証ツール（HTTPモード専用）](#認証ツール（httpモード専用）)
  - [ガイドツール（ツール発見）](#ガイドツール（ツール発見）)
  - [システム管理ツール](#システム管理ツール)
  - [MMP Meetingツール（SkillSet: mmp）](#mmp-meetingツール（skillset-mmp）)
  - [Multiuserツール（SkillSet: multiuser）](#multiuserツール（skillset-multiuser）)
- [使用例](#使用例)
  - [利用可能なスキルを一覧表示](#利用可能なスキルを一覧表示)
  - [ブロックチェーンステータスを確認](#ブロックチェーンステータスを確認)
  - [スキル遷移を記録](#スキル遷移を記録)
...
- [SkillSetプラグインアーキテクチャ](#skillsetプラグインアーキテクチャ)
  ...
  - [Multiuser SkillSet（マルチテナントユーザー管理）](#multiuser-skillset（マルチテナントユーザー管理）)
- [ディレクトリ構造](#ディレクトリ構造)
  ...
  - [リポジトリ構造（GitHubからクローン）](#リポジトリ構造（githubからクローン）)
- [Multiuser：マルチテナントユーザー管理](#multiuser：マルチテナントユーザー管理)
  - [Multiuser とは](#multiuser-とは)
  - [アーキテクチャ](#アーキテクチャ)
  - [前提条件](#前提条件)
  - [クイックスタート](#クイックスタート)
  - [MCP ツール](#mcp-ツール)
  - [コアフック（Option C：汎用フック）](#コアフック（option-c：汎用フック）)
...
- [アーキテクチャ](#アーキテクチャ)
  - [コンポーネント](#コンポーネント)
- [課金モデル](#課金モデル)
- [支払いフロー（証明中心設計）](#支払いフロー（証明中心設計）)
- [アンチシビル対策](#アンチシビル対策)
- [MCPツール](#mcpツール)
- [設定例](#設定例)
- [依存関係](#依存関係)
- [Synoptis：相互証明プロトコル (v3.5.0)](#synoptis：相互証明プロトコル-v350)
  - [Synoptis とは](#synoptis-とは)
  - [アーキテクチャ](#アーキテクチャ)
  - [クイックスタート](#クイックスタート)
  - [MCP ツール](#mcp-ツール)
  - [MMP 統合](#mmp-統合)
  - [信頼スコアリング](#信頼スコアリング)
  - [レジストリと構成的記録](#レジストリと構成的記録)
  - [ProofEnvelope 構造](#proofenvelope-構造)
...
- [順序制約](#順序制約)
- [哲学的根拠](#哲学的根拠)
- [インストラクションモード：`self_developer`](#インストラクションモード：self_developer)
- [将来：共同自己開発](#将来：共同自己開発)
- [SkillSet リリースチェックリスト](#skillset-リリースチェックリスト)
  - [1. README 用 L1 Knowledge の作成](#1-readme-用-l1-knowledge-の作成)
  - [2. README の再生成](#2-readme-の再生成)
...
- [FAQ](#faq)
  ...
  - [Q: L1知識の健全性をどう維持しますか？L1の肥大化をどう防ぎますか？](#q-l1知識の健全性をどう維持しますか？l1の肥大化をどう防ぎますか？)
  - [Q: Persona Assemblyとは何ですか？いつ使うべきですか？](#q-persona-assemblyとは何ですか？いつ使うべきですか？)
  - [Q: チーム利用の場合、APIへの拡張が必要ですか？](#q-チーム利用の場合、apiへの拡張が必要ですか？)
  - [Q: チーム運用でkairos.rbやkairos.mdの変更に投票システムは必要ですか？](#q-チーム運用でkairosrbやkairosmdの変更に投票システムは必要ですか？)
  - [Q: ローカルテストの実行方法は？](#q-ローカルテストの実行方法は？)
  - [Q: kairos.rbに含まれるメタスキルは何ですか？](#q-kairosrbに含まれるメタスキルは何ですか？)
  - [Q: L0スキルを変更するにはどうすればいいですか？手順は？](#q-l0スキルを変更するにはどうすればいいですか？手順は？)
  - [Q: L0 Auto-Checkとは何ですか？15項目チェックリストをどう助けますか？](#q-l0-auto-checkとは何ですか？15項目チェックリストをどう助けますか？)
  - [Q: KairosChainはどのような判断で自分のスキルを進化させようとしますか？そのためのメタスキルはありますか？](#q-kairoschainはどのような判断で自分のスキルを進化させようとしますか？そのためのメタスキルはありますか？)
  - [Q: スキル-ツール統一とは何ですか？Rubyファイルを編集せずにMCPツールを追加できますか？](#q-スキル-ツール統一とは何ですか？rubyファイルを編集せずにmcpツールを追加できますか？)
  - [Q: kairos.rb経由でツールを追加する場合と、tools/ディレクトリに直接追加する場合の違いは？](#q-kairosrb経由でツールを追加する場合と、toolsディレクトリに直接追加する場合の違いは？)
  - [Q: KairosChainはLLMに対してスキル作成を自発的に推奨すべきですか？](#q-kairoschainはllmに対してスキル作成を自発的に推奨すべきですか？)
  - [Q: スキルやナレッジ同士で矛盾が生じたらどうなりますか？](#q-スキルやナレッジ同士で矛盾が生じたらどうなりますか？)
  - [Q: StateCommitとは何ですか？監査可能性をどう向上させますか？](#q-statecommitとは何ですか？監査可能性をどう向上させますか？)
  - [Q: スキルが溜まりすぎたらどうなりますか？クリーンアップの仕組みはありますか？](#q-スキルが溜まりすぎたらどうなりますか？クリーンアップの仕組みはありますか？)
  - [Q: スキルが間違っていたり古くなっていたりした場合、どうやって修正しますか？](#q-スキルが間違っていたり古くなっていたりした場合、どうやって修正しますか？)
  - [Q: ファイルベースストレージとSQLiteの違いは何ですか？](#q-ファイルベースストレージとsqliteの違いは何ですか？)
  - [Q: Pure Agent Skillとは何ですか？なぜ重要ですか？](#q-pure-agent-skillとは何ですか？なぜ重要ですか？)
  - [Q: なぜKairosChainはRuby、特にDSLとASTを使うのですか？](#q-なぜkairoschainはruby、特にdslとastを使うのですか？)
  - [Q: ローカルスキルとKairosChainの違いは何ですか？](#q-ローカルスキルとkairoschainの違いは何ですか？)
- [サブツリー統合ガイド](#サブツリー統合ガイド)
  - [Q: P2Pスキル交換はどのように機能しますか？](#q-p2pスキル交換はどのように機能しますか？)
  - [Q: Knowledge-only制約とは何ですか？](#q-knowledge-only制約とは何ですか？)
  - [Q: 他のKairosChainインスタンスに接続するには？](#q-他のkairoschainインスタンスに接続するには？)
- [サブツリー統合ガイド](#サブツリー統合ガイド)
  - [なぜサブツリー（サブモジュールではなく）](#なぜサブツリー（サブモジュールではなく）)
  - [KairosChainレイヤーとの連携](#kairoschainレイヤーとの連携)
  - [セットアップ：新しいプロジェクトにKairosChainを追加](#セットアップ：新しいプロジェクトにkairoschainを追加)
  - [重要：サブツリー使用時のデータディレクトリ設定](#重要：サブツリー使用時のデータディレクトリ設定)
  - [日常の操作](#日常の操作)
  - [コンフリクトの解決](#コンフリクトの解決)
  - [マルチプロジェクト展開の例](#マルチプロジェクト展開の例)
  - [サブツリープル後：テンプレートの更新](#サブツリープル後：テンプレートの更新)
  - [リファレンス](#リファレンス)
...
KairosChainはModel Meeting Protocol（MMP）を通じてインスタンス間のピアツーピア知識交換をサポートしています。この機能はいくつかの重要な設計原則を体現しています：

**Knowledge-Only制約**：P2P経由で交換できるのは非実行コンテンツ（Markdown、YAML）のみです。実行可能コードは信頼されたチャネル経由でインストールする必要があります。この分離により、共有される知識の安全性と監査可能性を確保しつつ、コード実行のセキュリティ境界を維持します。

**プロトコル進化**：MMPは`ProtocolEvolution`を通じた動的プロトコル拡張をサポートし、コアプロトコルを変更せずに新しいメッセージタイプや機能を追加できます。拡張はコアアクションとの衝突を防ぐために検証され、プロトコルの整合性を維持します。

**ワイヤープロトコル標準化**：標準化されたメッセージフォーマット（tar.gz + Base64アーカイブ、SHA-256コンテンツハッシュ付き）により、内部構成に関係なくKairosChainインスタンス間の相互運用性を確保します。

**トラストモデル**：TOFU（Trust on First Use）とRSA-2048署名検証を組み合わせることで、中央認証局を必要とせずにID保証を提供し、KairosChainの分散型哲学に沿った設計となっています。
...
### Multiuserツール（SkillSet: multiuser）

Multiuser SkillSet がインストールされている場合に利用可能なツールです。PostgreSQL が利用できない場合でもツールは登録され、セットアップ状態の診断情報を提供します。

| ツール | 説明 |
|--------|------|
| `multiuser_status` | Multiuser SkillSet のステータス確認：PostgreSQL 接続、テナント数、ユーザー数、マイグレーション状態 |
| `multiuser_user_manage` | ユーザー管理：list、create、delete、update_role（owner のみ） |
| `multiuser_migrate` | データベースマイグレーション：status、run、dry_run（owner のみ） |

> **グレースフルデグラデーション**: PostgreSQL が利用できない場合、`multiuser_status` は診断レポート（`pg_gem_missing`、`pg_server_unavailable`、または `pg_error`）を対処方法付きで返します。他の KairosChain ツールは通常通り動作し続けます。
