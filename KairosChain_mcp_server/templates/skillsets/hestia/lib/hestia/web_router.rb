...
module Hestia
  ...
  class WebRouter
    ...
    def route_request(env, path)
      ...
      # ANC-7 public verification view (unauthenticated; digests + verification only)
      when '/place/web/verify'
        handle_web_verify(env)
      when %r{\A/place/web/anchor/([a-zA-Z0-9_.-]+)\z}
        handle_web_anchor($1)
    ...
    # --- ANC-7 public verification handlers ---
    ...
    def handle_web_anchor(id)
      ...
      html_response(200, render('web_anchor', record: record, place_name: place_name))
    end

    # Reconstruct the content-independent verification address from a URL slug.
    def anchor_source_id(slug)
      "place://#{anchor_place_host}/anchor/#{slug}"
    end

    def anchor_place_host
      @config['place_host'] ||
        (@config['place_url'] || '').sub(%r{\Ahttps?://}, '').sub(%r{/.*\z}, '')
    end
