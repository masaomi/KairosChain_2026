...
# SAME gated agent_step path with the recorded arguments. Every correctness
# property — per-session serialization, anchored at-most-once, side-effect
# intent bracket — is enforced by the AdvanceGate inside that call (Slice
# A-1), not by this script. If this worker dies, the driver re-issues the
# recorded call safely; if it double-runs, the gate serializes and replays.
#
# argv: <session_id> <session_dir>
# env:  KAIROS_PROJECT_ROOT (chdir target; .kairos discovery)
