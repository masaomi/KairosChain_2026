...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        err.message || 'ログインに失敗しました。メールアドレスとパスワードをご確認ください。'
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleGoogleAuth = async () => {
    ...
    setLoading(true);
    try {
      // In production, this would open a Google OAuth flow
      ...
      <div className="relative z-10 w-full max-w-md">
        <div className="glass-morphism rounded-2xl p-8 sm:p-10">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="text-right">
              <Link
                ...
                className="text-sm text-[#b0b0b0] hover:text-[#d4af37] transition-colors"
              >
                パスワードをお忘れですか？
              </Link>
            </div>
          ...
          >
            ...
            <span className="text-sm sm:text-base">Googleでログイン</span>
          </button>
