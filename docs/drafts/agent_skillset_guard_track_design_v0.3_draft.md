...
version: 0.3
status: DRAFT (post-R2 revision, pre-R3 verification round)
date: 2026-07-10
r2_fixes: >
...
# Agent SkillSet Guard Track — design v0.3
...
> (AGT-5), and with live-tree effects either quarantined in a scratch area
> exactly as on the delegated route, or pre-declared and refused outside the
> declaration (AGT-1) — route choice never relaxes the guard;
...
whose effects would reach the live tree either performs them in a
driver-declared scratch area with the same verdict-gated return, or has its
live-tree effect surface declared before the act and refused outside the
declaration. The deciding context influences which route an act takes, so
the routes must be comparably strict about pre-act gating — otherwise
steering an act in-process would escape the quarantine the delegated route
enforces (route-shopping), and the confinement would be only as strong as
the weaker route.
...
store write and a refused out-of-declaration live-tree effect from the
in-process route, a refused merge set containing a store path, a refused
non-record write through the recording path, a tamper halt — and additionally a
...
- Probes: blocked-write probe, blocked-read probe, overlap-declaration
  probe, no-spec-halt probe, out-of-declaration store-write and live-tree
  effect probes (in-process route), merge-store refusal probe (merge set
  containing a store path is refused), exemption-boundary probe (non-record
  write through the recording path is refused), tamper probe (spec hash
  mismatch), discrimination probe (non-conforming act must fail).
