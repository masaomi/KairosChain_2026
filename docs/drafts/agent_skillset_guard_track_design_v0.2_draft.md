...
version: 0.2
status: DRAFT (post-R1 revision + 4.6 sub-author pass, pre-R2)
date: 2026-07-10
sub_author_pass: >
...
# Agent SkillSet Guard Track — design v0.2
...
> (AGT-5); (ii) on either route, the cycle's success is judged against
> criteria the deciding context did not author, pinned before the act
> (AGT-3/AGT-4); and (iii) nothing reaches the stores except through the
> driver's admission. The decision itself remains governed by the existing
> policy gates and by the human at checkpoints.
...
One consequence is a deliberate behavior change: today the executor edits
live project files directly. Under this design it does not; it works in a
scratch area and its results return through the boundary (AGT-1). The loop's
observable outcomes are unchanged, but the write path moves.
...
the substrate level, not by convention or prompt. Legitimate results reach
...
evidence is state observed by the driver from boundary position — the scratch
area's actual content, the live runtime's actual effects — never the
executor's or the model's report of that state, which would let the act's own
participant supply the evidence for its own judgment — the same
self-assessment the mechanical verdict displaces. This ranges over *all* acts the loop performs,
in-process acts included: an act route changes how the act is confined, not
whether it is judged. Model-based reflection may continue as advisory signal,
...
level of shared conversation state, not merely of invocation. The committed
default channel: specifications are derived by the driver from acceptance
material declared in the mandate before the loop ran. Intent whose acceptance
...
loop ran, of which layers that mandate may touch. This declaration governs
writes to the governance stores. The driver's own constitutive recording —
writing the cycle's record to the chain and attestation stores — is an
inherent act of the boundary, not an admitted act: it is what makes the cycle
exist as a recorded event, and it cannot be subject to mandate-level
permission without circularity. Admission is refusal, not detection: a write outside the declared surface does not land, and the cycle
halts at a checkpoint with the refusal recorded. And admission is total for
the trusted runtime's loop machinery: no store-write path exists inside the
loop that does not pass through it — the absence of a bypass is itself part
...
merging results, and running the confinement probes at their declared
cadence — absence, inability to execute, an unmeasurable result, a
...
store write from the in-process route, a tamper halt — and additionally a
...
|---|---|---|
| INV-1 record-write boundary-only | AGT-1 | Localized: substrate-level write confinement, required disjointness, verdict-gated driver merge, standing probes |
...
| INV-4 mechanical, pre-authored acceptance | AGT-3 | Extended over both act routes; evidence provenance bound to boundary observation; reflection demoted to tighten-only advisory |
| INV-5 actor ≠ author of its judge | AGT-4 | Sharpened to deciding-context independence; default channel committed (mandate-derived, human fallback); decomposition ownership assigned |
| (external track: layer guard / admission) | AGT-5 | Imported with refusal semantics, structural attribution, and a no-bypass clause covering the in-process route |
...
## 8. Decisions resolved in R1, and what remains open

Resolved (committed in this version):

1. **Spec authorship channel** — mandate-derived by the driver, human at
   checkpoint for what the mandate cannot express, deciding context excluded
   (AGT-4). Alternates are backlog material requiring their own review.
2. **Advisory reflection** — retained, not removed: it is the loop's
   metacognitive channel, and the guard track's job is to remove it from the
   gate, not from the system's self-observation. Constrained to tighten-only
   and excluded from recorded success status (AGT-3).
3. **Work-area geometry** — scratch area with verdict-gated driver merge
   (AGT-1), accepting the behavior change from today's direct live-tree
   editing.
...
1. **Admission granularity (AGT-5).** Slice 1 commits to per-mandate layer
   ...
   declaration cost is left to operational evidence from Slice 1.
2. **Mandate acceptance-material expressiveness (AGT-4).** How much real
   ...
   Slice 1's attended runs are the measurement.
...
- Probes: blocked-write probe, blocked-read probe, overlap-declaration
  probe, no-spec-halt probe, out-of-declaration write probe (in-process
  route), tamper probe (spec hash mismatch), discrimination probe
