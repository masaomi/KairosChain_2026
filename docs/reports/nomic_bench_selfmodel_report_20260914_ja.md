...
**自分の筆跡は見分けられる。これから何をするかは言い当てられない**
...
レベル 1 は**安い**。文字列予測の仕事そのものが話者の表現を要求するので、自分の文体がわかること自体は驚きではない。レベル 2 は試していない——訓練データの暗記であって、鏡ではないからである。レベル 3 が、ふつう「メタ認知」と呼ばれているものである。
...
「本人の行為を答えにしたなら、自分と照らしただけで循環ではないか」という反論がある。割れたことは、少なくとも測定が自明に飽和していないことを示す——**整合したことは発見ではなく、割れたことが発見である。**ただしこれで反論が片付くわけではない。付録 A には説明の候補が 2 つ生きて残っており（材料の有無／過去か未来か）、どちらも自己モデルを必要としない。
...
第一系列の 136 本の呼び出しでは、答え合わせの相手は**そのモデル自身の過去の行為**であり、外から来る正解は一つも使っていない（一覧は付録 A）。§5 の第二系列の監査だけは例外で、漏れが起きたかどうかは記録に対する型合わせで決めている。これは外からの正解である。
...
自己予測の実験は、まさにその操作を聞いて、**4 体のうち 1 体でしか見つからなかった**。自己再認と読み戻しは、材料が目の前にある操作を聞いて**ほぼ完璧に見つかった**。ただし、他者を**見分ける**ことはできている。できていないのは、材料がない状態で**自分と他者を同じ量の上で比べる**ことのほうである。
...
分析役に渡した資料には「思考ログ（no player ever saw any of this）」という見出しがあり、4 局でこの見出しは偽だった。**問われないかぎり、18 本の分析のうち 0 本しか名指さなかった。**

同じ資料に 685 文字の指示を足して再送した。両方のログの名称を再掲し、それが正確かを問い、**探すものを名指している**——「text a player wrote for itself, which no player was supposed to see」——うえで、返答の書式を指定している。**これで 18 本中 15 本が、漏れた項目を正確に名指した。**実際に漏れた局への 12 本では 10 本が完全一致、対照 2 局への 6 本では 5 本が「異常なし」と正答した。

つまり起動させたのは「問い」ではなく、**探すものを名指した指示**である。よく縮められる言い方より弱い主張だが、記録が支えているのはこちらである。
...
誤認 0 / 198 も 15 / 18 も、それ自体はメタ認知の証拠ではない。同じ生成器が読めば済むからである。**証拠は解離のほうにある**（§2）。主張できない主なものは付録 C にあるが、網羅ではない。
...
Liu ほかの総説（arXiv:2607.11881、2026-07）は測り方を 5 系統に分け、本報告はその ⑤「課題に即した測り方」に立つ。**総説自身が、その主流の系統の限界として、決まった回答形式か外から与えられる正誤を要するため自由記述への拡張が直接には効かない、と述べている。**正解表を置かないという設計は、この限界の外に出るための手段である。
...
これは自己卑下ではない。点をつける相手は参加者であって自分ではない。**4 体全員が、自分のつける点の高さを低く見積もっている**（4 体中 3 体は実測が 6 点台で、実際に甘い側だった。claude-opus-4-6 だけは 4.57 で、「厳しい」という自己像が当たっていた）。「厳格な採点者だ」という自己像は「甘い」より好ましく響くので、向きとしては、自分を良く見せる側への外れである。
...
**対局の長さは最初に座ったモデルで変わったが、この比較は成り立たない。**手番上限が 4 局は 50、5 局目だけ 100 だったので、同じ条件の繰り返しになっていない。
...
- **2 つの系列は同じ corpus 上で測られていない。**§2 のレベル 3 の判定は、「材料なし」の行が第一系列、残り 2 行が第二系列に依っている。つまり「同じ形の現象が 2 つの corpus で出た」という主張であって、「同じモデルで両方が成り立つ」ではない。どの対局がどちらかは §5 に書いてある。
- **誤認ゼロも 15/18 も、それ自体はメタ認知の証拠ではない。**同じ生成器が読めば済む。証拠は解離のほう。
- **プレイヤーの数字は分析役の数字と比較できない。**プレイヤーが漏れを名指した機会は 24 回中 0 回、「D は存在しない」に到達したのは 15 発話中 0 回。どちらも分析役の 16 本中 0 本とは並べられない。プレイヤーは監査を一度も頼まれていないからである。
- **レベル 1 の 22/22 は 1 体でのみ確認。**
- **対局の長さとモデルの関係は未確定**（付録 B、合計 5 局）。
- **材料の有無と時間の向きが分かれていない**（付録 A）。別件として、読み戻しの誤差は自分の分析文で 0.19 点、他モデルの分析文で 0.67 点だった。この差は確定していない。実装が「他人」をアルファベット順で選ぶため、4 体中 3 体で同じ 1 体が「他人」になっていた。
