...
Ruby（DSL/AST）を採用した理由はここにある。「Skillを定義する」操作と「Skillの進化ルールを定義する」操作が、同じ言語・同じ構文・同じ実行環境で表現される。この構造的同型性が自己言及性の基盤であり、KairosChainの全ての性質はここから発芽する。

## 五つの命題（要約）

1. **生成的種子としての自己言及性** — 全てを演繹する公理ではなく、異なるレベルへの再帰的適用によって異なる特性を生むパターン。意図的に非対称：L0ガバナンスの核心部で最も強く、基盤実装では開かれたまま（「十分な自己言及性」）。
...
- **全文ドキュメント**: `docs/kairoschain_philosophy_claude_opus4.6_agent_team_20260223.md`
